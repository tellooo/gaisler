...
/* Define this if the BM initialization should wait for
 * a client to connect.
 */
...
int bm_loop(void)
{
	...
	time = rtems_clock_get_ticks_per_second() * 12;
	while ( time ) {
		/* Handle Log buffer */
		if ( bm_log(bm) ) {
...
int gr1553bm_test(void)
{
	...
	if ( bm_init(&bm, BM_LOG_BASE, ETHSRV_IP, ETHSRV_PORT, ETH_SERVER_WAIT_CLIENT) ) {
