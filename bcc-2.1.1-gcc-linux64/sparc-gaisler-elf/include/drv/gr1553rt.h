...
struct gr1553rt_cfg {
	...
	void *bd_sw_buffer;		/* Internal descriptor area.
					 * Must hold bd_count*4 bytes.
					 * If NULL, descriptors will be
					 * allocated dynamically. */
};
...
/* Calculate size of a RT list. Can be used to allocate memory */
#define GR1553RT_LIST_SIZE(bd_cnt) ( \
	offsetof(struct gr1553rt_list, bds) + \
	(bd_cnt) * (sizeof(unsigned short)) \
	)

/* Try to allocate and then call gr1553rt_list_init()
 *
 * \param rt       RT Device driver identification, stored within list.
 * \param plist    The list to configure. If NULL it will be allocated.
 * \param cfg      Configuration for list. Pointer to configuration is
 *                 stored within list for later use. Any buffer pointers
 *                 that are set to NULL will be allocated.
 */
...
/* Configure the RT.
 * The RT device must be configured once before
 * started. A started RT device can not be configured.
 *
 * Call gr1553rt_config_alloc() first if you want the driver to
 * allocate all the buffers pointers that are set to NULL in the
 * configuration.
 *
 * \param rt    The RT to configure
 * \param cfg   Configuration parameters
 */
extern int gr1553rt_config_init(void *rt, struct gr1553rt_cfg *cfg);

/* Allocate memory and configure the RT.
 * The RT device must be configured once before
 * started. A started RT device can not be configured.
 *
 * Call instead of gr1553rt_config_init()
 *
 * \param rt    The RT to configure
 * \param cfg   Configuration parameters
 */
extern int gr1553rt_config_alloc(void *rt, struct gr1553rt_cfg *cfg);

/* Free dynamically allocated buffers allocated by gr1553rt_config_alloc()
 *
 * \param rt    The RT to configure
 */
extern void gr1553rt_config_free(void *rt);
...
/* For accessing descriptors bypassing the driver */
extern unsigned short gr1553rt_bdid_from_list(
	struct gr1553rt_list *list,
	int entry_no
	);
extern struct gr1553rt_bd *gr1553rt_bd_from_list(
	struct gr1553rt_list *list,
	int entry_no);
extern struct gr1553rt_bd *gr1553rt_bd_from_bdid(void *rt, unsigned short bdid);
