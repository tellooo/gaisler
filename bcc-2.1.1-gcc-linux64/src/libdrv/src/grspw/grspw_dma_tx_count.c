...
/* Get number of transmitted packets not yet reclaimed. */
int grspw_dma_tx_count(struct grspw_dma_priv *dma)
{
        struct grspw_txring *tail;
        uint32_t hwdesc;
        uint32_t swtaildesc;
        int swtaildesc_en;
        int count = 0;
        ...
        swtaildesc_en = (tail->bd->ctrl >> GRSPW_TXBD_EN_BIT) & 0x1;

        hwdesc = dma->regs->txdesc;
        ...
        count = ((hwdesc - swtaildesc) & (BDTAB_SIZE - 1)) /
            (BDTAB_SIZE / GRSPW_TXBD_NR);

        /*
         * Handle the case when HW and SW pointer are equal because all TX
