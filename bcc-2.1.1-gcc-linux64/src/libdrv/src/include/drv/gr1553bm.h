 ...
 *
 * Call gr1553bm_config_alloc() first if you want the driver to
 * allocate all the buffers pointers that are set to NULL in the
 * configuration.
 */
extern int gr1553bm_config_init(void *bm, struct gr1553bm_config *cfg);

/* Allocate memory and configure BM driver before starting.
 *
 * Call instead of gr1553bm_config_init()
 *
 * \param bm    The BM to configure
 * \param cfg   Configuration parameters
 */
extern int gr1553bm_config_alloc(void *rt, struct gr1553bm_config *cfg);

/* Free dynamically allocated buffers allocated by gr1553bm_config_alloc()
 *
 * \param bm    The BM to configure
 */
extern void gr1553bm_config_free(void *bm);
