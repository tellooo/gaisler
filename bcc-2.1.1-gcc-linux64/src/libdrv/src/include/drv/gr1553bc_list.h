...
/* Calculate size of a BC list. Can be used to allocate memory */
#define GR1553BC_LIST_SIZE(max_major) ( \
	sizeof(struct gr1553bc_list) + \
	(max_major) * (sizeof(void *)) \
	)

/* Alloc and initialize a List with a maximum number of Major frames supported */
extern int gr1553bc_list_alloc(struct gr1553bc_list **list, int max_major);

/* Initialize an externally allocated list */
extern int gr1553bc_list_init(struct gr1553bc_list *list, int max_major);
...
/*  Calculate the worst case size of a minor table
 *  see gr1553bc_minor_table_size() for a more accurate function
 */
#define GR1553BC_MINOR_TABLE_SIZE(max_slots, timeslot) \
	(((max_slots) + 1 + ((timeslot) != 0)) * (GR1553BC_BD_SIZE))

/* Calculate the worst case size of a major table */
#define GR1553BC_MAJOR_TABLE_SIZE(max_minors, max_slots, timeslot) \
	((max_minors) * GR1553BC_MINOR_TABLE_SIZE(max_slots, timeslot))

/* Calculate the worst case size of a major table
 *  see gr1553bc_list_table_size() for a more accurate function
 */
#define GR1553BC_LIST_TABLE_SIZE(max_majors, max_minors, max_slots, timeslot) \
	((max_majors) * GR1553BC_LIST_TABLE_SIZE(max_minors, max_slots, timeslot))
...
/* Initialize an empty descriptor table from list description suitable for
 * the BC given by 'bc'.
 *
 * \param bdtab_custom   Custom Descriptor Allocation options:
 *                         ZERO: Invalid, use gr1553bc_list_table_alloc() instead
 ...
 */
extern int gr1553bc_list_table_init(
	struct gr1553bc_list *list,
	void *bdtab_custom
);

/* Allocate memory and initialize an empty descriptor table from list description suitable for
 * the BC given by 'bc'.
 *
 * \param bdtab_custom   Custom Descriptor Allocation options:
 *                         ZERO: Dynamically allocated by Driver (CPU near RAM)
 **                        Non-Zero: same behaviour as gr1553bc_list_table_init()
 */
...
/* Calculate size of a Major skeleton. Can be used to allocate memory */
#define GR1553BC_MAJOR_SIZE_SKEL(minor_cnt) ( \
	sizeof(struct gr1553bc_major) + \
	(minor_cnt)*(sizeof(struct gr1553bc_minor) + sizeof(void *)) \
	)

/* Alloc and initiialize a Major/Minor frame skeleton according to the configuration structure.
 * The descriptor table is not allocated.
 *
 * Call instead of  gr1553bc_major_init_skel()
 */
...
/* Initialize a Major/Minor frame skeleton according to the configuration structure.
 * The descriptor table is not allocated.
 */
extern int gr1553bc_major_init_skel(
	struct gr1553bc_major *major,
	struct gr1553bc_major_cfg *cfg
);

/* Free memory allocated by gr1553bc_major_alloc_skel() */
void gr1553bc_major_free_skel(struct gr1553bc_major *major);
