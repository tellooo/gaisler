...
static void gr1553bm_isr(void *data);
...
void *gr1553bm_open(int minor)
{
	...
	dev = gr1553_bm_open(minor);
	if ( dev == NULL )
		goto fail;

	priv = gr1553_bm_get(dev);
	if ( priv == NULL )
		goto fail;
...
void gr1553bm_close(void *bm)
{
	...
	gr1553_bm_close(priv->dev);
}

void gr1553bm_config_free(void *bm)
{
	struct gr1553bm_priv *priv = bm;
...
int gr1553bm_config_alloc(void *bm, struct gr1553bm_config *cfg)
{
	int retval = 0;
	struct gr1553bm_priv *priv = bm;

	/* Free old buffer if dynamically allocated */
	gr1553bm_config_free(priv);

	if (cfg->buffer_custom == NULL) {
		/* Allocate new buffer dynamically */
		priv->buffer = malloc(cfg->buffer_size + 8);
		if (priv->buffer == NULL) {
			return -1;
		}
	}

	if ((retval = gr1553bm_config_init(bm, cfg))) {
		gr1553bm_config_free(priv);
		return retval;
	}
...
int gr1553bm_config_init(void *bm, struct gr1553bm_config *cfg)
{
	int retval = 0;
	struct gr1553bm_priv *priv = bm;
...
/*#warning IMPLEMENT.*/
	...
	if ((unsigned int)cfg->buffer_custom & 1) {
		...
		if (cfg->buffer_custom == NULL) {
			 ...
			 */
			priv->buffer_base = (unsigned int)cfg->buffer_custom;
		}
...
int gr1553bm_start(void *bm)
{
	...
	if (osal_isr_register(&priv->irq, priv->dev->regs.interrupt, gr1553bm_isr, (void *)priv) != DRV_OK)
		return -3;
...
void gr1553bm_stop(void *bm)
{
	...
	osal_isr_unregister(&priv->irq, priv->dev->regs.interrupt, gr1553bm_isr, (void *)priv);
}
