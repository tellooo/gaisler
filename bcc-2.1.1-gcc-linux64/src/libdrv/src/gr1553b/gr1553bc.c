...
int gr1553bc_list_init(struct gr1553bc_list *list, int max_major)
{
	int size;

	size = GR1553BC_LIST_SIZE(max_major);
	memset(list, 0, size);
	list->major_cnt = max_major;
	 ...
	 *    required
	 */
	if (gr1553bc_list_config(list, &gr1553bc_def_cfg, NULL))
	{
		return -1;
	}
...
int gr1553bc_list_alloc(struct gr1553bc_list **list, int max_major)
{
	...
	*list = l;
	...
	if ( gr1553bc_list_init(l, max_major)) {
		free(l);
		return -1;
	}
...
static __inline__ union gr1553bc_bd *gr1553bc_bd_hw2cpu
	(
	struct gr1553bc_list *list,
...
int gr1553bc_list_table_alloc(struct gr1553bc_list *list, void *bdtab_custom)
{
	...
	if (bdtab_custom == NULL) {
		...
		list->_table = malloc(size + GR1553BC_BD_ALIGN);
		if ( list->_table == NULL ) {
			return -1;
		}
	}

	if ((retval = gr1553bc_list_table_init(list, NULL))) {
		gr1553bc_list_table_free(list);
	}
	return retval;
}

int gr1553bc_list_table_init(
	struct gr1553bc_list *list,
	void *bdtab_custom)
{
	struct gr1553bc_major *major;
	...
	if ((unsigned int)bdtab_custom & 0x1) {
		 ...
		 */
		list->table_hw = (unsigned int)((unsigned int)bdtab_custom & ~0x1);
		TRANSLATE_HW_TO_CPU(
			bcpriv->dev,
			list->table_hw,
			&list->table_cpu,
		...
		if (bdtab_custom == NULL) {
			...
			list->table_cpu = (unsigned int)bdtab_custom;
		}
...
)
{
	 ...
	 */
	minor = (struct gr1553bc_minor *)&major->minors[cfg->minor_cnt];
	for (i=0; i<cfg->minor_cnt; i++, minor++) {
		major->minors[i] = minor;
		minor->next = minor + 1;
	...
	)
{
	...
	size = GR1553BC_MAJOR_SIZE_SKEL(cfg->minor_cnt);
	maj = (struct gr1553bc_major *)malloc(size);
	if ( maj == NULL )
		return -1;


	if ((ret = gr1553bc_major_init_skel(maj, cfg))) {
		gr1553bc_major_free_skel(maj);
		return ret;
	}
	...
	uint16_t *dptr)
{
	...
	if ( (unsigned int)dptr & 0x1 ) {
		TRANSLATE_CPU_TO_HW(
			((struct gr1553bc_priv *)list->bc)->dev,
			(uint16_t *)((unsigned int)dptr & ~0x1),
			&dptr,
			-1
			);
	}
...
{
	/* Do nothing */
	(void)bd; /* Remove compiler warning (-Wunused-variable) */
	(void)data; /* Remove compiler warning (-Wunused-variable) */
}
...
void *gr1553bc_open(int minor)
{
	...
	dev = gr1553_bc_open(minor);
	if ( dev == NULL )
		goto fail;

	priv = gr1553_bc_get(dev);
	if ( priv == NULL )
		goto fail;
	...
	priv->regs = (struct gr1553b_regs *)dev->regs.addr;
	if (priv->regs == NULL)
		goto fail;
	...
	if (osal_isr_register(&priv->irq, priv->dev->regs.interrupt, gr1553bc_isr, (void *)priv) != DRV_OK)
		goto fail;
...
void gr1553bc_close(void *bc)
{
	...
	osal_isr_unregister(&priv->irq, priv->dev->regs.interrupt, gr1553bc_isr, (void *)priv);

	/* Free device */
	gr1553_bc_close(priv->dev);
