...
static void ambapp_add_scanned_bus(
	unsigned int *ioareas,
	unsigned int ioarea
	)
{
	int i;
...
static int ambapp_has_been_scanned(
	unsigned int *ioareas,
	unsigned int ioarea
	)
{
	...
	)
{
	...
	for (i = 0; i < maxloops; i++) {
		...
		if ( ahb_buf.id != 0 ){
			struct ambapp_dev_hdr _dev; struct ambapp_ahb_info _ahb;
			...
			DPRINTF((" = test %d:%d == %d:%d\n", vendor, device, dev->vendor, dev->device));
	...
	/* AHB SLAVES */
	ahb = (struct ambapp_pnp_ahb *) (ioarea | AMBA_CONF_AREA | AMBA_AHB_SLAVE_CONF_AREA);
	for (i = 0; i < maxloops; i++) {
		memcpy(&ahb_buf, ahb, sizeof(struct ambapp_pnp_ahb));
		if ( ahb_buf.id != 0 ){
			struct ambapp_dev_hdr _dev; struct ambapp_ahb_info _ahb;
			...
			if ( (dev->device == GAISLER_AHB2AHB) && (dev->vendor == VENDOR_GAISLER) ) {
				/* AHB/AHB Bridge Found, recurse down the Bridge */
				ahb_info = dev->devinfo;
				if ( ahb_info->ver ) {
					bridge_address = ambapp_addr_from(mmaps, ahb_info->custom[1]);

					DPRINTF(("+(AHBAHB:0x%x)\n",bridge_address));
					
					/* Makes sure bus only scanned once */
					if ( internal == 0 || ambapp_has_been_scanned(internal, bridge_address) == NULL) {
						if (internal)
			...
			}else if ( (dev->device == GAISLER_APBMST) && (dev->vendor == VENDOR_GAISLER) ) {
				...
				for (j=0; j<AMBA_APB_SLAVES; j++) {
					...
					if ( apb_buf.id ) {
						struct ambapp_dev_hdr _apbdev; struct ambapp_apb_info _apb;
						...
						DPRINTF((" = test %d:%d == %d:%d\n", vendor, device, apbdev->vendor, apbdev->device));
...
static int ambapp_dev_find_match(struct ambapp_dev_hdr *dev, void *arg)
{
...
static int find_apbslvs_next(int vendor, int device, struct ambapp_apb_info *dev, int index, int maxno)
{
...
int find_apbslv(int vendor, int device, struct ambapp_apb_info *dev)
{
...
void pnpinit(void) {
	struct ambapp_apb_info dev; int n;
	...
	if ((n = find_apbslv(VENDOR_GAISLER, GAISLER_GPTIMER, &dev)) == 1) {
		rtc = dev.start + 0x10;
                LEON3_GpTimer_Regs = (volatile LEON3_GpTimer_Regs_Map *) dev.start;
