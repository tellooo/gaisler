  ...
  if (
      ...
      ((mflat_proc) &&
       (!frame_id_eq (get_stack_frame_id (get_current_frame ()), 
		      step_frame_id) &&  frame_id_eq (frame_unwind_caller_id (get_current_frame ()), step_frame_id))))
...
insert_step_resume_breakpoint_at_frame (struct frame_info *return_frame)
{
  ...
  if (!return_frame) {
	  printf("insert_step_resume_breakpoint_at_frame: called with null frame\n");
	  return;
  }
...
insert_step_resume_breakpoint_at_caller (struct frame_info *next_frame)
{
  ...
  sr_sal.pc = ADDR_BITS_REMOVE(frame_unwind_caller_pc (next_frame));
  sr_sal.section = find_pc_overlay (sr_sal.pc);

  /*printf("insert_step_resume_breakpoint_at_caller 0x%x\n", sr_sal.pc);*/
