...
static void pelican_open(occan_priv *priv){
	...
	if ( ret ){
		...
		occan_calc_speedregs(priv->sys_freq_hz,priv->speed,&priv->timing);
	}
...
static int pelican_start(occan_priv *priv){
	...
	DBG("OCCAN: start: set timing regs btr0: 0x%x, btr1: 0x%x\n\r",READ_REG(priv, &priv->regs->bustim0),READ_REG(priv, &priv->regs->bustim1));

	/* Set default acceptance filter */
	pelican_set_accept(priv,priv->acode,priv->amask);

	/* Nothing can fail from here, this must be set before interrupts are enabled */
	priv->started = 1;

	/* turn on interrupts */
	WRITE_REG(priv, &priv->regs->inten,
		PELICAN_IE_RX | PELICAN_IE_TX | PELICAN_IE_ERRW | PELICAN_IE_ERRP | PELICAN_IE_BUS);
