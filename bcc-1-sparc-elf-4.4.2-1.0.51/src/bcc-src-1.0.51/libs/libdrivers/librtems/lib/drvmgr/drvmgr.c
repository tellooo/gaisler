 ...
 */
void drvmgr_insert_dev_into_drv(
 ...
 * The minor number is found by looking for a gap or at the end.
 */
void drvmgr_insert_dev_into_bus(
	struct drvmgr_bus *bus,
	struct drvmgr_dev *dev)
{
	struct drvmgr_dev *curr = 0, **pprevnext;
	int minor;
	...
	DBG("+++++Insert %p into bus %p %p\n", dev, bus, bus->children);
	...
	DBG("---- %p n:%p curr b:%p\n", dev, dev->next_in_bus, bus->children);
	*pprevnext = dev;
