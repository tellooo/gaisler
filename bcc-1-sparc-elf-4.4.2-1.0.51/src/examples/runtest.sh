...
while getopts vhr12x:p:o:c:sl:tm:k: OPT
do
    case "$OPT" in
	v)
	    verbose="yes";;
...
do
   ...
   echo " o image             : $tname"
   echo " o cmd               : $simbin $extra_options -c ${scriptfile} $tfile "
   echo " o scriptfile        : ${scriptfile} "
   echo " o logfile           : ${infofile} "
   echo " o max runtime       : $max_run_time "
   echo " o controlscript     : ${controlscript} "
   if [ "$controlscript" != "" ]
   then
   echo "  => " perl runtest.pl -f $controlscript  -l ${logfile}.tmp 
   fi
   fi
   ...
   sim_pid=$!
       ...
       mkfifo testpipe_in
       #(: > testpipe) &
       <testpipe_in $simbin $extra_options  -c ${scriptfile} $tfile >testpipe_out &
       sim_pid=$!
       <testpipe_out python -u runtest.py $controlscript  2>testpipe_in | tee ${logfile}.tmp &
       #) | tee ${logfile}.tmp &
       ctrl_pid=$!
       sleep 1
       echo -n " " > testpipe_in
...
#	   while true 
#	   do
#	       read var1; echo ${var1} > testpipe_in;
#	   done
#       } 
   fi
   ...
   # Make sure it won't run forever...
   {
       ...
       while [ $max_run_time -eq -1 -o $time_run -lt $max_run_time ]
       do
           ...
           # if a control application is running exit when the control app exits
           if [ "$ctrl_pid" != "" ]
           then
               if kill -0 $ctrl_pid 2>/dev/null
               then
	           echo >/dev/null
               else
	           break
       ...
       { sleep 1; kill -9 $sim_pid 2>/dev/null; } &
       if [ $killbin != "" -a "$verbose" = "yes" ]; then
	   sim_pid=$(/sbin/pidof $killbin );
	   kill -2 $sim_pid 2>/dev/null
           { sleep 1; kill -9 $sim_pid 2>/dev/null; } &
       fi
   ...
   if [ "$ctrl_pid" != "" ]
   then
       kill -2 $ctrl_pid 2>/dev/null
       { sleep 1; kill -9 $ctrl_pid 2>/dev/null; } &
   fi
